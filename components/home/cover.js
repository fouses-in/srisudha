...
export default function Covers() {
  ...
  return (
    <div>
      <div className="flex z-[60] mx-auto gap-6 px-4 py-4 -mt-40 max-w-[1232px]">
        {[
          {
            image: "cover1.jpeg",
          ...
          <div
            ...
            className={`testimonialC flex  z-50  flex-shrink-0 ${
              open == index ? "flex-[2]" : "flex-1"
            } p-2 cursor-pointer bg-white`}
          >
            <div
              className={`${
                index == open ? "w-1/2" : "w-full"
            ...
            <div
              className={`overflow-hidden transition-all duration-200 ${
                open == index
                  ...
                  : "flex-0 text-opacity-0"
              }`}
            >
              <p
                className={`font-medium ${
                  open == index ? "block" : "hidden"
                } transition-all delay-150 leading-7 text-[22px] `}
              >
                {value.label}
              </p>
              <p
                className={`text-gray-400 ${
                  open == index ? "block" : "hidden"
                } transition-all delay-150 mt-4 text-[15px] font-light font-Roboto`}
              >
                {value.value}
              </p>
              <button className=" font-medium flex items-center gap-2 text-white self-start mt-6">
